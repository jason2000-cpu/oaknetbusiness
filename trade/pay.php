...
$app=new App;
 ...
 foreach($account_balance as $account){
...
<!-- if the subscription already exists get back to the page -->
...
// update the new account balance
 $query = "UPDATE accounts SET account_balance = :balance WHERE account_name = :owner";
    $arr = [
...
// then add it to trading amount
 $query="INSERT INTO trading(owner,principal)VALUES(:owner,:principal)";
   $arr=[
       ":owner"=>$username,
       ":principal"=>$amount,
...
//    get the trading acounts for this guy
$query="SELECT * FROM trading WHERE owner='{$username}' AND principal='{$amount}'";
$trading_accounts=$app->select_all($query);
foreach($trading_accounts as $trading_account){
    $traded_date=$trading_account->traded_at;
}

// then calculate the intrest earned eacch month
// Example usage:
$principal_amount = $amount;   
$annual_rate = $rate;         
$created_at = $traded_date; 
$monthly_interest =$app->calculate_monthly_interest($principal_amount, $annual_rate, $created_at);
// the intrest the update
// the account after the intrest
$moneyearned=$principal_amount + $monthly_interest;
// the update query
// update the new account balance
 $query = "UPDATE trading SET amount = :amount ,intrest=:intrest WHERE owner = :owner AND principal =:principal";
 $arr = [
     ':amount' => $moneyearned, 
     ':owner' => $username,
     ':principal' => $amount,
     ':intrest' => $monthly_interest,
 ];
 $app->updateToken($query, $arr);
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="me-breadcrumb-box">
                            ...
                            <p><a href="<?php echo APPURL ?>/home.php">Home</a>Trading</p>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <div class="me-my-account-profile">
                            <div class="me-my-profile-head">
                                <div class="me-profile-name">
                                    <h4>Thank You for Trading With US</h4>
                                </div>
                                <div class="me-my-profile-img">
                                    <div class="me-my-profile-img-main">
                                        <img src="../assets/images/logoo-removebg-preview.png" class="img-fluid"
                                            alt="image">
                            ...
                            <div class="me-my-profile-body">
                                <div class="me-account-summary">
                                    <div class="me-account-summary-head">
                                        <div>
                                            <h4>Your Amount will be Earned monthly for more information contact us via
                                                the contact page</h4>
                                        </div>
                                    </div>
                                    <div class="me-account-summary-body me-account-money-detail">
                                        <ul>
                                            <a href="http://localhost/trade/home.php" class="me-btn">
                                                Go back to home</a>
                                            </li>
        ...
        <div class="me-breadcrumb">
            <div class="container">
                <div class="row">
                    <div class="col-12">
                        <div class="me-breadcrumb-box">
                            <h1>Subscription Exists</h1>
                            <p><a href="<?php echo APPURL ?>/home.php">Home</a>Subsctiptions</p>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <div class="me-my-account-profile">
                            ...
                            <div class="me-my-profile-body">
                                <div class="me-account-summary">
                                    <div class="me-account-summary-head">
                                        <div>
                                            <h4>It seems like your account balance is not enough for you to trade with
                                                that amount please recharge your amount your account page.For furthere
                                                assistance contact us in the the contact page or feel free to visit our
                                                offices for furthere assistance</h4>
                                        </div>
                                    </div>
                                    <div class="me-account-summary-body me-account-money-detail">
                                        <ul>
                                            <a href="../home.php" class="me-btn">
                                                Go back to home</a>
                                            </li>
