...
// the logic to handle insert and update
...
$account_bal_after_inv = $transactionAmount + $accountBalance;


if ($accountBalance === 0) {
    // If the account balance is zero, insert a new row with the initial balance
    $query = "INSERT INTO accounts (account_name, account_balance) VALUES (:owner, :amount)";
    $arr = [
        ":owner" => $accountOwner,
        ":amount" => $account_bal_after_inv,
    ];

    $app->insertWithoutPath($query, $arr);
} else {
    // Otherwise, update the accounts table with the new balance for the specified account owner
    $query = "UPDATE accounts SET account_balance = :amount WHERE account_name = :owner";
    $arr = [
        ":amount" => $account_bal_after_inv,
        ":owner" => $accountOwner,
    ];
